  ...
  return next(authReq).pipe(
    catchError(err => {
      if (err instanceof HttpErrorResponse && err.status === 401) {
        if (isRefreshing) return refreshSubject.pipe(filter(t => t !== null), take(1), switchMap(t => next(req.clone({ setHeaders: { Authorization: `Bearer ${t!}` } }))));
...
export const errorInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  return next(req).pipe(
    catchError((err: HttpErrorResponse) => {
      ...
      else if (err.status === 404) toast.showError('Introuvable', err.error?.message ?? 'Introuvable.');
      else if (err.status >= 500) toast.showError('Erreur serveur', 'Erreur interne.');
