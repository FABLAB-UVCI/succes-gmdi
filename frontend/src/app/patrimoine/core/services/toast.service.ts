...
export interface Toast {
  id: string; message: string; title?: string;
...
export class ToastService {
  ...
  showError(title: string, msg: string): void { this._push('_err_' + Date.now(), msg, title, 'error'); }
  ...
  private _push(id: string, message: string, title: string | undefined, type: Toast['type']): void {
    ...
    setTimeout(() => {
      this.toasts.update(t => { const c = { ...t }; if (c[id]) c[id] = { ...c[id], visible: false }; return c; });
